...
void WaterUtility::calculateRestrictions(int year, int week, int numdays, int month, int realization, int lag_weeks)
{
	...
	if(usesROF) // use ROF for owasa and raleigh
	{
		...
		while (restrictionsOff[restrictionStage] > ROF_res)  { // vice versa for removing restriction stages
			...
			if(restrictionStage == 0) {
				lag_on = true;
				week_of_last_restrictions = (year-1)*52+week-1;
			}
		}
	...
	else if (lag_on && ((year-1)*52+week-1) - week_of_last_restrictions < lag_weeks)
	{
		// If during lag period, treat as though stage 1 even though restrictions are off
		restrictionReduction = reductions[month-1][1];
		restrictedDemand = weeklyDemand*restrictionReduction;
		currentCostPerMG = costPerMG[month-1+(year-1)*12][0];
		
		// Revenues lost due to restrictions
		demandDeficit = weeklyDemand*baselineCostPerMG - restrictedDemand*currentCostPerMG;
		weeklyDemand = restrictedDemand; // update actual demand
		Fund.subtract(demandDeficit); // dip into the insurance fund to cover demand deficit
	}
	else
	{
