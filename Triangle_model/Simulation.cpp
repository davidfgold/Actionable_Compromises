...
void Simulation::calculateWaterPrices(double elasticity[])
{
	durham.calcWaterPrice(elasticity);
	cary.calcWaterPrice(elasticity);
	raleigh.calcWaterPrice(elasticity);
	owasa.calcWaterPrice(elasticity);
}
void Simulation::calculateWaterSurcharges(double elasticity[])
{
	durham.calcSurchargePrice(elasticity, 0);
	owasa.calcSurchargePrice(elasticity, 0);
	cary.calcSurchargePrice(elasticity, 0);
	raleigh.calcSurchargePrice(elasticity, 0);
}
...
{	
	...
	if (borgToggle ==3)
	{
		//cout << "inside the calc! rank = " << solutionNumber << " " << endl;
		for (int i = 1; i < num_dec+1; i++)
		{
			xreal[i-1] = parameterInput[solutionNumber][i];
			//cout << xreal[i-1] << " ";
		}
		//cout << endl;
		for (int r = 0; r < num_rdm_factors; r++)
		{
			rdm_factors[r] = RDMInput[rdmNumber][r];
		}
	}
	else
	{
	...
	// Update factors based on RDM values
	
	// Demand factors go into price calculations
	preconditionData(rdm_factors[5], rdm_factors[6], false);
	
	double elasticity_scaled[5];
	for (int i = 0; i < 5; i++)
		elasticity_scaled[i] = elasticity_Of_Demand[i]*rdm_factors[2];
		
	calculateWaterPrices(elasticity_scaled);
	calculateWaterSurcharges(elasticity_scaled);
	
	// Consumer reductions factor (0/1 is irrigation/non, and 0-4 is the restriction stage
	durham.individualReductions[0][0] = 1;
	durham.individualReductions[0][1] = .85*rdm_factors[3];
	durham.individualReductions[0][2] = .7*rdm_factors[3];
	durham.individualReductions[0][3] = .6*rdm_factors[3];
	durham.individualReductions[0][4] = .45*rdm_factors[3];
	durham.individualReductions[1][0] = 1;
	durham.individualReductions[1][1] = .93*rdm_factors[3];
	durham.individualReductions[1][2] = .88*rdm_factors[3];
	durham.individualReductions[1][3] = .83*rdm_factors[3];
	durham.individualReductions[1][4] = .7*rdm_factors[3];
	
	owasa.individualReductions[0][0] = 1;
	owasa.individualReductions[0][1] = .9*rdm_factors[3];
	owasa.individualReductions[0][2] = .85*rdm_factors[3];
	owasa.individualReductions[0][3] = .8*rdm_factors[3];
	
	raleigh.individualReductions[0][0] = 1;
	raleigh.individualReductions[0][1] = .923*rdm_factors[3];
	raleigh.individualReductions[0][2] = .84615*rdm_factors[3];
	raleigh.individualReductions[0][3] = .53846*rdm_factors[3];
	raleigh.individualReductions[0][4] = .384615*rdm_factors[3];
	
	//Cary reductions by stage
	cary.individualReductions[0][0] = 1;
	cary.individualReductions[0][1] = .87*rdm_factors[3];
	cary.individualReductions[0][2] = .68*rdm_factors[3];
	cary.individualReductions[0][3] = .62*rdm_factors[3];
	cary.individualReductions[0][4] = .54*rdm_factors[3];
	cary.individualReductions[1][0] = 1;
	cary.individualReductions[1][1] = .94*rdm_factors[3];
	cary.individualReductions[1][2] = .9*rdm_factors[3];
	cary.individualReductions[1][3] = .83*rdm_factors[3];
	cary.individualReductions[1][4] = .71*rdm_factors[3];
	
	transferCosts = rdm_factors[12];
	
	durham.insurancePremium = rdm_factors[13];
	cary.insurancePremium = rdm_factors[13];
	raleigh.insurancePremium = rdm_factors[13];
	owasa.insurancePremium = rdm_factors[13];
	...
	correlateDemandVariations(rdm_factors[7]);
...
{	
	...
	double falls_lake_supply_capacity = 14700.0*rdm_factors[8];
	double falls_lake_wq_capacity = (14700.0 - falls_lake_supply_capacity) + 20000.0;
	double jordan_lake_supply_capacity = 14924.0*rdm_factors[9];
	double jordan_lake_wq_capacity = (14924.0 - jordan_lake_supply_capacity) + 30825.0;
	double cary_treatment_capacity = 40.0*rdm_factors[10];
	double durham_cary_capacity = 10.0*rdm_factors[11];
	double durham_owasa_capacity = 7.0*rdm_factors[11];
	double raleigh_cary_capacity = 10.8*rdm_factors[11];
	
	for (int realization = 0; realization < numRealizations; realization++)
	{
		if (flags.timing == "on") timing.startRandom = clock();
		...
		if (flags.timing == "on")
		{
			timing.endRandom = clock();
			timing.timeComm += double(timing.endRandom)-double(timing.startRandom);
		}
		...
		while (year-1<(terminateYear))
		{
			...
			durham.calculateRestrictions(year, week, numdays, month, realization, int(rdm_factors[4]));
			owasa.calculateRestrictions(year, week, numdays, month, realization, int(rdm_factors[4]));
			cary.calculateRestrictions(year, week, numdays, month, realization, int(rdm_factors[4]));
			raleigh.calculateRestrictions(year, week, numdays, month, realization, int(rdm_factors[4]));
			...
			durhamActualInflow = durhamInflows.synthetic[realization][syntheticIndex]*rdm_factors[0];
			owasaActualInflow = owasaInflows.synthetic[realization][syntheticIndex]*rdm_factors[0];
			fallsActualInflow = fallsInflows.synthetic[realization][syntheticIndex]*rdm_factors[0];
			wbActualInflow = wheelerInflows.synthetic[realization][syntheticIndex]*rdm_factors[0];
			claytonActualInflow = claytonInflows.synthetic[realization][syntheticIndex]*rdm_factors[0];
			crabtreeActualInflow = crabtreeInflows.synthetic[realization][syntheticIndex]*rdm_factors[0];
			jordanActualInflow = jordanInflows.synthetic[realization][syntheticIndex]*rdm_factors[0];
			lillingtonActualInflow = lillingtonInflows.synthetic[realization][syntheticIndex]*rdm_factors[0];
		
			actualEvap = durhamOwasaEvap.synthetic[realization][syntheticIndex]*rdm_factors[1];
			actualFallsEvap = fallsEvap.synthetic[realization][syntheticIndex]*rdm_factors[1];
			actualWBEvap = wheelerEvap.synthetic[realization][syntheticIndex]*rdm_factors[1];
			...
			{	
				...
				if (year == 6)
					systemStorage.upgradeCaryTreatmentPlant(rdm_factors[10]);
			}
